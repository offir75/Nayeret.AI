...
import { ArrowRight, ArrowLeft, Trash2, Sparkles, ChevronDown } from 'lucide-react';
...
export const ReviewStep = ({ mode, pages, bundleName, onBundleNameChange, onDelete, onFinish, onBack }: ReviewStepProps) => {
  ...
  const [taxTagged, setTaxTagged] = useState(false);
  const [transactionType, setTransactionType] = useState<'expense' | 'income' | 'neutral'>('expense');
        ...
        <Collapsible open={financialOpen} onOpenChange={setFinancialOpen}>
          <CollapsibleTrigger asChild>
            <button className="flex items-center justify-between w-full p-3 rounded-lg border border-border hover:bg-muted/50 transition-colors">
              <span className="text-sm font-medium">
                {lang === 'en' ? 'Financial Options' : 'אפשרויות פיננסיות'}
              </span>
          ...
          <CollapsibleContent className="pt-4 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="tax-tagged" className="text-sm">
                {lang === 'en' ? 'Tag for tax reporting' : 'תייג לדיווח מס'}
              </Label>
              <Switch id="tax-tagged" checked={taxTagged} onCheckedChange={setTaxTagged} />
            </div>

            {/* Transaction Type */}
            <div className="space-y-2">
              <Label className="text-sm">
                {lang === 'en' ? 'Transaction Type' : 'סוג תנועה'}
              </Label>
              <Select value={transactionType} onValueChange={(v) => setTransactionType(v as 'expense' | 'income' | 'neutral')}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="expense">
                    {lang === 'en' ? '💸 Expense' : '💸 הוצאה'}
                  </SelectItem>
                  <SelectItem value="income">
                    {lang === 'en' ? '💰 Income' : '💰 הכנסה'}
                  </SelectItem>
                  <SelectItem value="neutral">
                    {lang === 'en' ? '📄 Neutral' : '📄 ניטרלי'}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
        ...
        {pages.length === 1 && mode === 'single' ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}
            ...
            <div className="relative w-full max-w-md h-full rounded-2xl overflow-hidden border border-border/50 bg-card shadow-lg">
              <img src={pages[0].dataUrl} alt={t('pageWord')} className="w-full h-full object-contain" />
            ...
            <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.06 } } }} className="grid grid-cols-2 gap-3 pb-4">
              {pages.map((page, i) => (
                <motion.div key={page.id} variants={gridItem} className="relative rounded-xl overflow-hidden border border-border/50 bg-card group">
                  <img src={page.dataUrl} alt={`${t('pageWord')} ${i + 1}`} className="w-full aspect-[3/4] object-cover" />
                  <motion.div className="absolute inset-x-0 h-[1px] bg-gradient-to-l from-transparent via-primary/40 to-transparent pointer-events-none"
