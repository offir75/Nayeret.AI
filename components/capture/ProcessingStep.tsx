...
import { CheckCircle2, ScanLine, Calendar, Banknote, FileText, Coins, CalendarDays, FileCheck, X, Plus } from 'lucide-react';
...
export const ProcessingStep = ({ pageCount, pageNames, bundleName, onDone, onScanMore }: ProcessingStepProps) => {
  const isMulti = pageCount > 1 && pageNames && pageNames.length > 1;
  if (isMulti) return <MultiProcessing pageNames={pageNames!} bundleName={bundleName} onDone={onDone} onScanMore={onScanMore} />;
  return <SingleProcessing pageCount={pageCount} onDone={onDone} onScanMore={onScanMore} />;
};

const SingleProcessing = ({ pageCount, onDone, onScanMore }: { pageCount: number; onDone: () => void; onScanMore?: () => void }) => {
  const { t } = useLanguage();
    ...
    <div className="flex-1 flex flex-col items-center px-6 py-8 overflow-y-auto min-h-0">
      <AnimatePresence mode="wait">
        {!done ? (
          ...
          <SuccessView particles={particles} pageCount={pageCount} onDone={onDone} onScanMore={onScanMore} />
        )}
      </AnimatePresence>
    </div>
  );
...
const MultiProcessing = ({ pageNames, bundleName, onDone, onScanMore }: { pageNames: string[]; bundleName?: string; onDone: () => void; onScanMore?: () => void }) => {
  const { t } = useLanguage();
  ...
  if (allDone) {
    return (
      <div className="flex-1 flex flex-col items-center px-6 py-8 overflow-y-auto min-h-0">
        <SuccessView particles={particles} pageCount={completedCount} bundleName={bundleName} onDone={onDone} onScanMore={onScanMore} />
      </div>
    );
  }
...
const SuccessView = ({ particles, pageCount, bundleName, onDone, onScanMore }: {
  particles: Particle[]; pageCount: number; bundleName?: string; onDone: () => void; onScanMore?: () => void;
}) => {
  const { t } = useLanguage();
  return (
    <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="flex flex-col items-center text-center w-full max-w-md relative">
      <div className="absolute top-10 left-1/2 -translate-x-1/2 pointer-events-none">
        {particles.map((p) => { const Icon = p.icon; return (
          <motion.div key={p.id} className="absolute" initial={{ opacity: 0.9, x: 0, y: 0, scale: p.scale, rotate: 0 }}
            animate={{ opacity: 0, x: p.x, y: p.y, rotate: p.rotation }} transition={{ duration: 1.8, delay: p.delay, ease: 'easeOut' }}>
            <Icon className="w-4 h-4 text-primary/50" />
          </motion.div>
        ); })}
      </div>
      <motion.div initial={{ scale: 0 }} animate={{ scale: [0, 1.25, 1] }} transition={{ times: [0, 0.6, 1], duration: 0.6, ease: 'easeOut' }}
        className="w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center mb-6 relative z-10">
        <motion.div className="absolute inset-0 rounded-full border-2 border-primary/30" initial={{ scale: 1, opacity: 0.6 }} animate={{ scale: 1.5, opacity: 0 }} transition={{ duration: 0.8, delay: 0.3, ease: 'easeOut' }} />
        <CheckCircle2 className="w-12 h-12 text-primary" />
      </motion.div>
      <motion.h2 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.3, ease: 'easeOut' }} className="text-2xl font-bold text-foreground mb-2">{t('allReady')}</motion.h2>
      <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.3, ease: 'easeOut' }} className="text-muted-foreground text-sm mb-10">
        {bundleName ? `${bundleName} — ` : ''}{pageCount} {pageCount === 1 ? t('processedSuccessSingle') : t('processedSuccess')}
      </motion.p>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.3 }} className="w-full space-y-3 sticky bottom-0 pt-4 pb-[env(safe-area-inset-bottom)] bg-gradient-to-t from-background via-background to-transparent">
        <Button onClick={onDone} className="h-14 px-8 text-base font-semibold gap-2 w-full">{t('backToMyDocs')}</Button>
        {onScanMore && (<Button onClick={onScanMore} variant="outline" className="h-12 px-6 text-sm gap-2 w-full"><Plus className="w-4 h-4" />{t('scanMore')}</Button>)}
      </motion.div>
    </motion.div>
  );
};
